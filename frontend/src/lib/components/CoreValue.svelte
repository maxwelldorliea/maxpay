<script>
  const secure = 'images/shield.svg';
  const fees = 'images/dolar.svg';
  const seemless = 'images/seemless.svg';
  const fun = 'images/fun.svg';
</script>

<div class="relative pt-20 sm:pt-24 lg:pt-32">
  <h1 class="tracking-tight font-extrabold text-4xl sm:text-5xl lg:text-6xl text-slate-900 text-center dark:text-slate-600">Why MAXPAY?</h1>
</div>
<div class="flex justify-evenly pt-16 sm:20 lg:pt-28">
  <figure class="rounded-xl p-8 md:p-0 dark:bg-white">
  <img class="w-24 h-24 md:w-48 md:h-auto md:rounded-none rounded-full mx-auto" src={secure} alt="Secure Icon" >
  <div class="pt-6 md:p-8 text-center">
     <h3 class="text-2xl font-semibold tracking-tight mb-2">Security And Privacy</h3>
     <p class="text-lg font-medium">
        Build from ground up with privacy and security in mind
     </p>
  </div>
</figure>
  <figure class="rounded-xl p-8 md:p-0 dark:bg-white">
  <div>
    <img class="w-24 h-24 md:w-48 md:h-auto md:rounded-none rounded-full mx-auto" src={fees} alt="Dollar Icon">
  </div>
  <div class="pt-6 md:p-8 text-center">
     <h3 class="text-2xl font-semibold tracking-tight mb-2">Pay Less</h3>
     <p class="text-lg font-medium">
        Pay little to no fees with no hidden fees
     </p>
  </div>
</figure>
</div>
<div class="flex justify-evenly">
  <figure class="rounded-xl p-8 md:p-0 dark:bg-white">
  <img class="w-24 h-24 md:w-48 md:h-auto md:rounded-none rounded-full mx-auto" src={seemless} alt="Secure Icon" >
  <div class="pt-6 md:p-8 text-center">
     <h3 class="text-2xl font-semibold tracking-tight mb-2">Seemless And Reliable</h3>
     <p class="text-lg font-medium">
        Build from ground up to be secure and reliable.
     </p>
  </div>
</figure>
  <figure class="rounded-xl p-8 md:p-0 dark:bg-white">
  <div>
    <img class="w-24 h-24 md:w-48 md:h-auto md:rounded-none rounded-full mx-auto" src={fun} alt="Dollar Icon">
  </div>
  <div class="pt-6 md:p-8 text-center">
     <h3 class="text-2xl font-semibold tracking-tight mb-2">Fun To Use</h3>
     <p class="text-lg font-medium">
        Build from ground up to be easy to use.
     </p>
  </div>
</figure>
</div>
<style lang="postcss">
</style>
